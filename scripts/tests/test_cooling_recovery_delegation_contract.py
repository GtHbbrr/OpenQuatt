...
YAML, RUNTIME, CORE, DISPATCH, SAFETY_POLICY, ACTUATOR_RUNTIME, ACTUATOR_YAML, SUBSTITUTIONS, DEFROST_TEST = (
    (ROOT / path).read_text() for path in ("openquatt/oq_cooling_strategy.yaml", "openquatt/includes/control/oq_cooling_runtime.h", "openquatt/includes/control/oq_cooling_demand_logic.h", "openquatt/includes/control/oq_cooling_dispatch_logic.h", "openquatt/includes/control/oq_cooling_safety_policy.h", "openquatt/includes/control/oq_thermal_actuator_runtime.h", "openquatt/oq_thermal_actuator.yaml", "openquatt/oq_substitutions_common.yaml", "tests/host/oq_odu_compressor_levels_test.cpp"))
class CoolingRecoveryDelegationContractTest(unittest.TestCase):
    def test_yaml_delegates_and_live_safety_paths_stay_fail_closed(self) -> None:
        for marker in ("demand_runtime().tick", "DemandInput input", "input.sensor_valid = supply_valid && target_valid",
                       "dispatch_tick(dispatch)", "DispatchInput dispatch", "publish_limiter_event_"): self.assertIn(marker, RUNTIME)
        for marker in ("oq_cooling_runtime::runtime().demand_tick", "oq_cooling_runtime::runtime().dispatch_tick"):
