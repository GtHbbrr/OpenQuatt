...
|---|---:|---|
| HP1 Modbus | `${oq_modbus_startup_delay_ms}` (default 0ms) | Fast poll every `${oq_modbus_update_interval_s}` (default 10s) plus slow control-readback poll every third cycle (30s); hub turnaround `${oq_modbus_command_throttle_ms}` (default 100ms) |
