...
Quick Start toont na keuze van de verwarmingsstrategie een aparte aanbeveling voor **Warmtevraag bepalen**: bij `Power House` adviseert OpenQuatt `Heating Enable Source = Niet gebruiken` (OpenQuatt bepaalt zelf de vraag), bij `Water Temperature Control` adviseert OpenQuatt de thermostaat als warmtetoestemming (`OT thermostat` op Q-edition, anders `CIC`/`HA input`). De web-app overschrijft een bestaande keuze bij strategieswitch niet stil; er verschijnt alleen een `Aanbevolen instelling` / `Controleer configuratie` melding met een knop om het advies over te nemen. Afwijkende combinaties blijven bewust mogelijk (bijv. Power House met zone-gate, stooklijn volledig weersafhankelijk).
...
Voor `Heating Enable Source` betekent `Niet gebruiken` / `Disabled`: geen externe warmtetoestemming gebruiken; de actieve verwarmingsstrategie mag zelf warmtevraag opbouwen. Dit staat dus niet voor verwarming uitschakelen. Bij `Power House` is dit meestal gewenst; bij `Water Temperature Control` met kamerthermostaat is meestal `OT thermostat` gewenst. Zie [Power House](power-house.md) en [Water Temperature Control](water-temperature-control.md).
