          ...
          >
            ...
            <span class="oq-settings-mqtt-sensor-status-cell">
              <em
                ...
                aria-label="${s(T)}: ${s(k)}"
              >${s(q)}</em>
              ${y?`<span class="oq-settings-mqtt-sensor-retained" title="${s(H)}" aria-label="${s(H)}">R</span>`:""}
            </span>
            <button
  ...
  `}function sS(){if(!o.root)return;if(o.interfacePanelOpen){f();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=Zl();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function lS(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=Gn()),r&&(r.textContent=hi())}function Xl(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function cS(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${Xl(e)}</span>`}function no(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${Xl(e)}</span>`}function uS(){return[...Vo]}function dS(){let e=[];return Vo.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function pS(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function bi(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function gr(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function Ho(e){return`${De()}/openquatt/debug-recording/${e}`}function Ur(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function ec(){Ur({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function mS(){let e=await window.fetch(Ho("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Ur(t),t}function Qn(e=2e3){gr(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{wi({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function wi(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",f());try{await mS(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),Qn()}catch(t){ec(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?f():(sS(),lS())}}async function _m(){let e=Hl(Vo,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch(Ho(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==Vo.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${Vo.length})`);return t}async function gS(e){let t=Math.max(1,Number(e)||15);bi(),gr(),Ua({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),f();try{await _m();let r=await window.fetch(Ho(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();Ur(n),Qn()}catch(r){ec(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,f()}}async function fS(){bi(),gr(),Ua({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),f();try{await _m();let e=await window.fetch(Ho("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();Ur(t),Qn()}catch(e){ec(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,f()}}async function hS(){let e=await window.fetch(Ho("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Ur(t),gr(),t}async function bS(){bi(),o.debugRecordingBusy=!0,o.debugRecordingError="",f();try{await hS(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,f()}}async function wS(e={}){bi(),gr(),o.debugRecordingBusy=!0,o.debugRecordingError="",f();try{let t=await window.fetch(Ho("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();Ur(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,f()}}function Cm(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(Zu/1e3),sample_count:It(),column_count:Vo.length,storage:"browser"},columns:uS(),units:dS(),initial:pS(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function gi(e){return JSON.stringify(e)}function yS(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([gi(Cm())]).size}catch{return gi(Cm()).length}}function vS(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function SS(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function qS(){if(It()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",f();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Ft();f();try{let t=await window.fetch(Ho("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,Vs(SS(r),gi(r),"application/json"),Rm(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,f()}}async function kS(){if(It()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",f();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Ft();f();try{let t=await window.fetch(Ho("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await wo(gi(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");Rm(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,f()}}var TS={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",f(),wi()),"start-debug-recording":e=>gS(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>fS(),"select-debug-recording-duration":e=>aS(e.dataset.debugMinutes||15),"stop-debug-recording":()=>wS(),"freeze-debug-recording":()=>bS(),"download-debug-recording":()=>qS(),"copy-debug-recording":()=>kS()};function Dm(e,t){return se(TS,e,t)}function Pm(){let e=o.debugRecordingActive,t=Ft(),r=fi(),n=It(),a=o.debugRecordingBusy,i=vS(yS()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=nS(),d=Am(),u=xm(),m=iS(),w=n>0,y=`${m.toFixed(1)}%`,h=[{icon:"status",label:"Status",value:Gn()},{icon:"clock",label:t?"Retentie":"Duur",value:mr(t?u:$m())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],g=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return te({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:hi(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
    ...
    `}function Vm(){let e=o.apiSecurityStatus||{},t=e.enabled===!0,r=!!e.key,n=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active),a=r||e.transport_active===!0||n,i=o.apiSecurityNotice,l=o.apiSecurityError?`<div class="oq-helper-modal-note oq-helper-modal-note--error" aria-live="assertive">${s(o.apiSecurityError)}</div>`:"";return te({id:"system",titleId:"oq-api-security-modal-title",kicker:"Toegang",title:OS(),copy:LS(),className:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit API-beveiliging popup",body:`
      ...
      `;return te({id:"system",titleId:"oq-login-modal-title",kicker:"Systeem",title:PS(),copy:NS(),closeAction:"close-system-modal",closeLabel:"Sluit login-popup",body:`
        ...
        ${n?`<button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="save-web-auth" ${o.authBusy?"disabled":""}>${t?"Opslaan":"Login opslaan"}</button>`:""}`})}function ea(e,t,r){let n=R(t),a=R(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=R(e);return Number.isFinite(i)?i:NaN}function nc(e="oq-settings-grid"){let t=WS();return`
      <div class="${s(e)}">
    ...
    `}function VS({status:e,running:t,resultReady:r,startDisabled:n,abortDisabled:a,applyDisabled:i,busy:l,controlsAvailable:c}){let d=String(e||"").toUpperCase(),u=d.includes("FAILED")||d.includes("REFUSED")||d.includes("ABORT"),m=d.includes("APPLIED"),w=v("hp2WaterIn")||v("hp2WaterOut")||v("hp2WaterInRaw")||v("hp2WaterOutRaw"),y=R("hpWaterCalibrationStableProgress"),h=R("hpWaterCalibrationStableRequired"),g=R("hpWaterCalibrationRemaining"),p=Math.round(R("hpWaterCalibrationPhase")),b=t&&(p===1||d.includes("MIXING")),S=t&&!b,q=300,T=60,k=Number.isFinite(g)?Math.max(0,q-g):NaN,M=Number.isFinite(k)?Math.max(0,T-k):NaN,H=b&&Number.isFinite(k)?Math.max(0,Math.min(100,k/T*100)):S&&Number.isFinite(y)&&Number.isFinite(h)&&h>0?Math.max(0,Math.min(100,y/h*100)):t&&Number.isFinite(g)?Math.max(0,Math.min(100,100-g/q*100)):r?100:0,_=r&&v("hpWaterCalibrationResultSpreadBefore")?Te("hpWaterCalibrationResultSpreadBefore",2):Te("hpWaterCalibrationSpread",2),W=b?"Water mengen":Number.isFinite(y)&&Number.isFinite(h)&&h>0?y>0?`${Math.round(Math.max(0,y))} / ${Math.round(h)} s binnen grenzen`:"Nog niet binnen grenzen":"Wachten op stabiel venster",I=r?3:t?2:1,j=m?"Offsets toegepast":r?`Meting klaar - spreiding ${_}`:t?b?`Water mengen${Number.isFinite(M)&&M>0?` - meting start over ${Math.round(M)} s`:""}`:`Meting bezig - ${Number.isFinite(g)&&g>0?`max. ${Math.round(g)} s resterend`:W}`:u?"Meting niet voltooid":"Voorbereiding",x=m?"De voorgestelde offsets zijn opgeslagen.":r?"Controleer de voorgestelde offsets en pas ze toe.":t?b?"De waterpomp circuleert zonder compressor zodat de watertemperaturen eerst kunnen mengen.":"De firmware stopt zodra het laatste meetvenster binnen de spreiding- en driftgrenzen valt.":u?rr("hpWaterCalibrationStatus","Controleer de voorwaarden en start opnieuw."):w?"Start alleen wanneer compressor en boiler uit zijn. HP1 en HP2 water in/out worden samen naar een relatieve referentie gebracht.":"Start alleen wanneer compressor en boiler uit zijn. Bij single setup wordt HP1 water in/out onderling gelijkgetrokken; supply blijft diagnose.",P=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",liveKey:"hp1WaterIn",resultRawKey:"hpWaterCalibrationResultHp1InRawAvg",offsetKey:"hp1WaterInOffset",suggestedKey:"hp1WaterInOffsetSuggested"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",liveKey:"hp1WaterOut",resultRawKey:"hpWaterCalibrationResultHp1OutRawAvg",offsetKey:"hp1WaterOutOffset",suggestedKey:"hp1WaterOutOffsetSuggested"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",liveKey:"hp2WaterIn",resultRawKey:"hpWaterCalibrationResultHp2InRawAvg",offsetKey:"hp2WaterInOffset",suggestedKey:"hp2WaterInOffsetSuggested"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",liveKey:"hp2WaterOut",resultRawKey:"hpWaterCalibrationResultHp2OutRawAvg",offsetKey:"hp2WaterOutOffset",suggestedKey:"hp2WaterOutOffsetSuggested"}].filter(O=>v(O.liveKey)||v(O.rawKey)||v(O.offsetKey)),D=(O,V)=>{let X=I>O;return`
      ...
      `,J=O=>{let V=R(O.resultRawKey),X=Number.isFinite(V)?V:ea(O.rawKey,O.liveKey,O.offsetKey),Ne=R(O.suggestedKey),Et=Number.isFinite(X)&&Number.isFinite(Ne)?eo(X+Ne,o.entities[O.suggestedKey]?.uom||"\xB0C",2):"\u2014";return`
        <tr>
    ...
    `}function Um(){let e=v("boilerCvAssistEnabled")&&C("boilerCvAssistEnabled"),t=ai(),r=C("cm100Active"),n=String(t||"").trim().toUpperCase(),a=fr(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,d=!!o.pendingCommissioningCm100Start,u=c||i||a,m=c||!i,w=nr("boilerPowerTestStatus","IDLE"),y=qi(w,"boiler"),h=C("boilerPowerTestActive"),g=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",p=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),b=!!o.pendingBoilerPowerTestStart,S=o.commissioningTaskLock==="boiler",q=fr(w),k=!Ce(w)&&(h||b||S||Yr(w))&&!q,M=He("boilerRatedHeatPower"),H=He("boilerHeatPower"),_=R("boilerHeatPower"),W=_>0?H:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:H;_>0&&(o.commissioningBoilerHeatPowerDisplay=H);let I=nr("flowAutotuneStatus","IDLE"),j=qi(I,"autotune"),x=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",P=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),D=!!o.pendingFlowAutotuneStart,N=o.commissioningTaskLock==="autotune",J=fr(I),V=!Ce(I)&&(D||N||Yr(I))&&!J,X=nr("airPurgeStatus","IDLE"),Ne=qi(X,"purge"),Et=C("airPurgeActive"),ft=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",lo=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),Ct=!!o.pendingAirPurgeStart,Oe=o.commissioningTaskLock==="purge",de=!Ce(X)&&(Et||Ct||Oe||Yr(X)),wn=/DONE/.test(String(X||"").toUpperCase()),xr=!!(lo||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),$=He("airPurgeRemaining",{decimals:0}),A=R("airPurgePhase"),z=A===1?"Rustig":A===2?"Pulsen":A===3?"Stabiliseren":Ne.phase,B=nr("manualFlowStatus","IDLE"),ee=C("manualFlowActive"),ne=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",me=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),ge=!!o.pendingManualFlowStart,ce=o.commissioningTaskLock==="manual-flow",fe=!Ce(B)&&(ee||ge||ce||Yr(B)),Ye=nr("manualHpStatus","IDLE"),co=C("manualHpActive"),Re=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",uo=!!(o.entities.manualHpStart||o.entities.manualHpAbort),po=!!o.pendingManualHpStart,Be=o.commissioningTaskLock==="manual-hp",xe=!Ce(Ye)&&(co||po||Be||Yr(Ye)),yn=/SAFETY STOP/.test(String(Ye||"").toUpperCase()),Ke=/STOPPING/.test(String(Ye||"").toUpperCase()),go=nr("hpWaterCalibrationStatus","IDLE"),Db=qi(go,"hp-water-calibration"),Iu=C("hpWaterCalibrationActive"),Ca=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",Fu=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),xa=!!o.pendingHpWaterCalibrationStart,fo=o.commissioningTaskLock==="hp-water-calibration",Je=!Ce(go)&&(Iu||xa||fo||Yr(go)),$a=/DONE|APPLIED/.test(String(go||"").toUpperCase()),Wu=/APPLIED/.test(String(go||"").toUpperCase()),Pb=He("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),Nb=He("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),ju=/DONE|APPLIED/.test(String(w||"").toUpperCase()),Vu=/DONE|APPLIED/.test(String(I||"").toUpperCase()),bs=i?q?"Wachten op CM100":k?y.phase:ju?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",ws=i?J?"Wachten op CM100":V?j.phase:Vu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",ys=i?de?Ne.phase:wn?"Klaar":"Klaar om te starten":"Wachten op CM100",Bu=i?fe?"Actief":"Klaar om te starten":"Wachten op CM100",Ku=i?xe?Ke?"Bezig met stoppen":yn?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",zu=i?Je?Db.phase:Wu?"Offsets toegepast":$a?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Uu=!i||g||!p||V||de||fe||xe||Je||k||N||Oe||ce||Be||fo||b,Ob=g||!(k||S||b),Lb=g||Uu||!ju||V||de||Je,Gu=!i||x||!P||k||de||fe||xe||Je||V||S||Oe||ce||Be||fo||D,Ib=x||!(V||N||D),Fb=x||Gu||!Vu||k||de||Je,Wb=!i||ft||!lo||k||V||fe||xe||Je||de||S||N||ce||Be||fo||Ct,jb=ft||!(de||Oe||Ct),Vb=!i||ne||!me||k||V||de||xe||Je||fe||S||N||Oe||Be||fo||ge,Bb=ne||!(fe||ce||ge),Kb=!i||Re||!uo||k||V||de||fe||Je||xe||S||N||Oe||ce||fo||po,zb=Re||!(xe||Be||po),Ub=!i||Ca||!Fu||k||V||de||fe||xe||Je||S||N||Oe||ce||Be||xa,Gb=Ca||!(Je||fo||xa),Qb=Ca||Je||!$a||Wu;d&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),b&&(h||Ce(w))&&(o.pendingBoilerPowerTestStart=!1),S&&Ce(w)&&(o.commissioningTaskLock=""),D&&Ce(I)&&(o.pendingFlowAutotuneStart=!1),N&&Ce(I)&&(o.commissioningTaskLock=""),Ct&&(Et||Ce(X))&&(o.pendingAirPurgeStart=!1),Oe&&Ce(X)&&(o.commissioningTaskLock=""),ge&&(ee||Ce(B))&&(o.pendingManualFlowStart=!1),ce&&(ee||Ce(B))&&(o.commissioningTaskLock=""),po&&(co||Ce(Ye))&&(o.pendingManualHpStart=!1),Be&&(co||Ce(Ye))&&(o.commissioningTaskLock=""),xa&&(Iu||Ce(go))&&(o.pendingHpWaterCalibrationStart=!1),fo&&Ce(go)&&(o.commissioningTaskLock="");let Yb=a?"Wachten op CM100":t,Jb=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",Zb=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:zu,available:!!(Fu||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Jr({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:zu,statusCopy:Je?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":$a?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:VS({status:go,running:Je,resultReady:$a,startDisabled:Ub,abortDisabled:Gb,applyDisabled:Qb,busy:Ca,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:Bu,available:!!(me||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Jr({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:Bu,statusCopy:fe?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:Ku,available:!!(uo||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Jr({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:Ku,statusCopy:xe?Ke?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":yn?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
          ...
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:ws,available:!0,openDisabled:fr(ws),cardMarkup:Jr({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:ws,statusCopy:J?"Wacht totdat CM100 actief is voordat je autotune start.":V?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ${o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort?ar({active:V,startKey:"flowAutotuneStart",stopKey:"flowAutotuneAbort",startLabel:"Autotune starten",stopLabel:"Autotune stoppen",startDisabled:x||Gu,stopDisabled:x||Ib}):""}
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:Xt(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,_o(),o.appView==="results"&&wr({force:!0}).then(()=>f())})},"select-energy-history-import-file":()=>qq(),"clear-energy-history-import-file":()=>{hg(),f()},"import-energy-history-file":()=>kq(),"export-energy-history":()=>lq(),"open-history-storage-modal":()=>{o.settingsStoragePage="overview",o.systemModal="history-storage",f();let e=Di({forceMissing:!0,forceTrendHistory:!0,forceDecisionLog:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&f()});return _o([1e3,3e3,7e3]),e},"open-storage-diagnosis":()=>{o.settingsStoragePage="diagnosis",f()},"open-storage-decision-log":()=>{o.settingsStoragePage="decision-log",f()},"open-storage-energy":()=>{o.settingsStoragePage="energy",f()},"back-storage-overview":()=>{o.settingsStoragePage="overview",f()},"download-settings-backup":()=>Aq(),"open-settings-backup-import":()=>{o.settingsBackupMqttPassword="",o.settingsBackupRestoreResult=null,o.settingsBackupError="",o.systemModal="settings-backup-import",f()},"confirm-settings-backup-restore":()=>Pq()};function $g(e,t){return se(Nq,e,t)}function yc(e,t,r="",n=!1){return`
    ...
    `}function Ng(){let e=["diagnosis","decision-log","energy"].includes(o.settingsStoragePage)?o.settingsStoragePage:"overview",t=v("trendHistoryEnabled")&&C("trendHistoryEnabled"),r=t&&v("trendHistoryFlashEnabled")&&C("trendHistoryFlashEnabled"),n=v("decisionLogHistoryEnabled"),a=n&&C("decisionLogHistoryEnabled"),i=_g(),l=Rg(i.storedEvents),c=v("lifetimeEnergyHistoryEnabled"),d=c&&C("lifetimeEnergyHistoryEnabled"),u=c?je("lifetimeEnergyHistoryAvailable","Geen data"):"Geen data",m=Ag(u),w=v("lifetimeEnergyHistoryClear")&&!["Geen data","\u2014"].includes(u),y=t&&v("trendHistoryFlush"),h=a&&v("decisionLogHistoryFlush"),g=v("lifetimeEnergyHistoryCapture"),p=Dg(),b=String(o.energyHistoryRaw||"").includes("@hour_retention|"),S=b&&!p.hourPartitionAvailable,q=b?S?"Alleen live":Hg(p.hourStoredDayCount,"Geen uurdata"):"Laden...",T=b&&!S?Li(p.hourStorageKb):"\u2014",k=b&&!S?sa(p.hourWriteCount):"\u2014",M=b&&!S?Oi(p.hourLastWriteTimestampS):"Geen data",H=e==="overview"?"":`
      ...
      <p class="oq-settings-storage-footnote"><strong>Goed om te weten:</strong> gegevens die worden bewaard, blijven beschikbaar na een herstart. Tijdelijke gegevens bestaan alleen zolang de controller online is.</p>`;return e==="diagnosis"?(x="Diagnosegegevens",P="Beheer technische meetreeksen voor diagnosegrafieken en hulp bij problemen.",D=`${H}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
      ...
      </section>${vc([W])}`):e==="decision-log"?(x="Beslisloghistorie",P="Bewaar exacte controllerkeuzes en gebeurtenissen, maximaal zeven dagen.",D=`${H}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
        <div class="oq-settings-storage-domain-rows">
    ...
    `;return he("curveControlProfile","Regelprofiel","Kies of de stooklijn vooral comfortabel, gebalanceerd of rustig moet reageren.",n,"oq-settings-field--span-2")}function Jq(){let e=(N,J=0)=>{let O=R(N);return Number.isNaN(O)?J:Math.max(0,O)},r=e("phComfortBelow",.1),n=e("phComfortAbove",.3),a=e("phKp",3e3),i=20-r,l=20+n,c=620,d=184,u=46,m=24,w=18,y=40,h=96,g=c-u-m,p=Math.min(20-1.2,i-.35),b=Math.max(20+1.2,l+.35),S=N=>u+(N-p)/Math.max(.01,b-p)*g,q=S(p),T=S(b),k=S(i),M=S(20),H=S(l),_=Math.abs(i-20)>.001,W=Math.abs(l-20)>.001,I=w+24,j=d-y,x=h-44,P=(N,J,O,V="")=>{let Et=Math.max(q+4,Math.min(T-110-4,N-55)),ft=N-14,lo=x,Ct=28,Oe=h-x+16;return`
      ...
      `},D=[`M ${q.toFixed(1)} ${I.toFixed(1)}`,`L ${k.toFixed(1)} ${h.toFixed(1)}`,`L ${H.toFixed(1)} ${h.toFixed(1)}`,`L ${T.toFixed(1)} ${j.toFixed(1)}`].join(" ");return`
      <div class="oq-ph-concept-card">
        <div class="oq-ph-concept-visual">
          ...
          <svg class="oq-ph-concept-svg" viewBox="0 0 ${c} ${d}" role="img" aria-label="Grafiek voor Power House tuning">
            ...
            <rect x="${k.toFixed(1)}" y="${w}" width="${Math.max(20,H-k).toFixed(1)}" height="${(d-w-y).toFixed(1)}" rx="18" class="oq-ph-concept-band oq-ph-concept-band--calm"></rect>
            <rect x="${H.toFixed(1)}" y="${w}" width="${Math.max(20,T-H).toFixed(1)}" height="${(d-w-y).toFixed(1)}" rx="18" class="oq-ph-concept-band oq-ph-concept-band--above"></rect>
            ...
            ${W?`<line x1="${H}" y1="${h-12}" x2="${H}" y2="${h+12}" class="oq-ph-concept-marker oq-ph-concept-marker--above"></line>`:""}
            ...
            ${W?`<circle cx="${H}" cy="${h}" r="5" class="oq-ph-concept-point oq-ph-concept-point--above"></circle>`:""}
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(E("hpGeneration")||""),n=Ot(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
  ...
  `}function lk(){let e=oe(),t=lt(),r=`${e}:${t}`,n=o.quickStartSetupDraft||r,[a,i]=n.split(":");return{...pi(a,i),currentKey:r,selectedKey:n,changes:n!==r,targetIsDuo:a==="duo",targetIsEthernet:i==="eth"}}function ck(){let e=lk(),t=qt(),r=!!(t||o.updateInstallBusy),n=[["single:wifi","Single \xB7 Wi-Fi","E\xE9n warmtepomp via het draadloze netwerk."],["single:eth","Single \xB7 Ethernet","E\xE9n warmtepomp via een vaste netwerkkabel."],["duo:wifi","Duo \xB7 Wi-Fi","Twee warmtepompen via het draadloze netwerk."],["duo:eth","Duo \xB7 Ethernet","Twee warmtepompen via een vaste netwerkkabel."]],a=[e.targetIsDuo?"De tweede warmtepomp is aangesloten en hoort bij deze controller.":"Deze controller wordt voor \xE9\xE9n warmtepomp gebruikt.",e.targetIsEthernet?"De netwerkkabel is aangesloten.":"De Wi-Fi-gegevens zijn beschikbaar op de controller."];return`
    ...
    `,u=(T,k,M="Actief",H="Normaal",_={})=>{if(!v(T))return"";let W=Z(T);return d({label:k,value:W?M:H,active:_.warningWhenActive&&W||_.warningWhenInactive&&!W})},m=(T,k,M={})=>{let H=M.fallbackKey||"";return!v(T)&&!(H&&v(H))?"":d({label:k,value:He(v(T)?T:H,M)})},w=(T,k)=>{let M=k.filter(Boolean).join("");return M?`
